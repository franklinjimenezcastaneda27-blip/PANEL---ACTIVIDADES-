...
document.addEventListener("DOMContentLoaded", () => {
  ...
  State.subscribe((newState) => {
    ...
    if(!document.getElementById('modalUsers').classList.contains('hidden')){ UI.renderAdminUsersModal(newState.users, toggleUserStatus); }
    if(!document.getElementById('modalAudit').classList.contains('hidden')){ UI.renderAdminAuditModal(newState.auditLogs); }
  });
});
...
function fetchData() {
  ...
  FirebaseService.listenEvents((data) => {
    eventsLoaded = true; State.updateData(data, null, null, null); checkLoader(eventsLoaded, absLoaded);
  });
  
  FirebaseService.listenAbsences((data) => {
    absLoaded = true; State.updateData(null, data, null, null); checkLoader(eventsLoaded, absLoaded);
  });

  if(State.currentUser && State.currentUser.rol === 'admin') {
     FirebaseService.listenAudit((logs) => { State.updateData(null, null, null, logs); });
  }
}

function checkLoader(e, a) { if(e && a) document.getElementById("appLoader").classList.add('hidden'); }
...
function setupEventListeners() {
  ...
  document.getElementById("globalSearch").addEventListener("input", (e) => {
    clearTimeout(searchTimeout); searchTimeout = setTimeout(() => { State.setFilters(undefined, undefined, e.target.value); UI.renderSearch(State, calendarActions); }, 400);
  });
  ...
  document.getElementById("btnAdminUsers").addEventListener("click", () => { UI.renderAdminUsersModal(State.users, toggleUserStatus); UI.showModal("modalUsers"); });
  document.getElementById("btnAdminAudit").addEventListener("click", () => { UI.renderAdminAuditModal(State.auditLogs); UI.showModal("modalAudit"); });
  ...
  document.getElementById("btnSaveUser").addEventListener("click", async () => {
    ...
    try {
      await FirebaseService.saveUser({ nombre, email, rol }); UI.showToast("Usuario guardado");
  ...
  // NUEVO: Botón de Actualizar Gestión (Foto como Base64 100% Gratis y GPS)
  document.getElementById("btnUpdateStatus").addEventListener("click", async () => {
    ...
    btn.textContent = "Obteniendo GPS..."; btn.disabled = true;
    ...
    try {
      ...
      } catch(e) { UI.showToast("No se pudo obtener el GPS", "warning"); }

      btn.textContent = "Procesando foto...";
      let photoData = null;
      if(fileInput.files.length > 0) {
         // Comprime la foto localmente para no gastar espacio en base de datos
      ...
      UI.showToast("Gestión guardada exitosamente"); UI.closeModals();
    } catch(error) {
      UI.showToast("Error al guardar gestión", "error");
    } finally {
      btn.innerHTML = "💾 Guardar Gestión"; btn.disabled = false; fileInput.value = "";
    }
  });
  
  document.getElementById("btnDeleteEvent").addEventListener("click", async () => {
    if(confirm("¿Eliminar actividad permanentemente?")) {
      await FirebaseService.deleteEvent(currentSelectedEvent.id, currentSelectedEvent.title); UI.closeModals(); UI.showToast("Eliminado");
    }
  });
  ...
  document.getElementById("btnDeleteAbsence").addEventListener("click", async () => {
    if(confirm("¿Seguro que deseas eliminar este registro?")) {
      ...
      await FirebaseService.deleteAbsence(currentSelectedAbsenceId, abs.responsible); UI.closeModals(); UI.showToast("Registro eliminado");
    }
  });
  document.getElementById("eventType").addEventListener("change", (e) => {
    if(!document.getElementById("eventId").value) document.getElementById("eventRepeatGroup").classList.toggle("hidden", e.target.value !== "cobranza_comunal");
  });
}

// FUNCION MAGICA: Comprime fotos para guardarlas gratis en la base de datos de Firestore
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = event => {
      ...
      img.onload = () => {
        ...
        const MAX_WIDTH = 500; // Ancho máximo pequeñito para que no pese
        const scaleSize = MAX_WIDTH / img.width;
        canvas.width = MAX_WIDTH;
        canvas.height = img.height * scaleSize;
        const ctx = canvas.getContext('2d');
        ctx.drawImage(img, 0, 0, canvas.width, canvas.height);
        // Devuelve un string largo de texto (Calidad 60%)
...
async function toggleUserStatus(id, newStatus) { try { await FirebaseService.saveUser({ activo: newStatus }, id); UI.showToast("Actualizado"); } catch(e) { UI.showToast("Error", "error"); } }
...
function openNewEventModal(eventObj = null) {
  ...
  if(eventObj) {
    ...
    let h = parseInt(hh); document.getElementById("eventAmPm").value = h >= 12 ? 'PM' : 'AM'; document.getElementById("eventHour").value = h % 12 || 12; document.getElementById("eventMin").value = mm;
    document.getElementById("eventRepeatGroup").classList.add("hidden");
  } else {
    document.getElementById("eventResponsible").value = ""; document.getElementById("eventHour").value = "9"; document.getElementById("eventMin").value = "00"; document.getElementById("eventAmPm").value = "AM";
...
function openEventActionsModal(id) {
  ...
  document.getElementById("eventNewStatus").value = ev.estado || (ev.completed ? 'exitosa' : 'pendiente');
  
  UI.showModal("modalEventActions");
}

function openNewAbsenceModal() { document.getElementById("absDate").value = Cal.formatDateISO(State.selectedDate); document.getElementById("absObs").value = ""; document.getElementById("absResponsible").value = ""; UI.showModal("modalFormAttendance"); }
function openAbsenceActionsModal(id) {
  const abs = State.absences.find(a => a.id === id); if(!abs) return; currentSelectedAbsenceId = id;
...
async function saveEvent() {
  const id = document.getElementById("eventId").value; const title = document.getElementById("eventTitle").value.trim(); const dateVal = document.getElementById("eventDate").value; const respVal = document.getElementById("eventResponsible").value;
  if(!title || !dateVal || !respVal) return UI.showToast("Completa datos obligatorios", "error");
  let h = parseInt(document.getElementById("eventHour").value); const m = document.getElementById("eventMin").value; const ampm = document.getElementById("eventAmPm").value;
  if(ampm === "PM" && h !== 12) h += 12; if(ampm === "AM" && h === 12) h = 0;
  const timeVal = `${h.toString().padStart(2, '0')}:${m}`;
  const baseData = { title, time: timeVal, responsible: respVal, type: document.getElementById("eventType").value, completed: false, estado: 'pendiente' };
  const btn = document.getElementById("btnSaveEvent"); btn.textContent = "Guardando..."; btn.disabled = true;
  try {
    if (id) { await FirebaseService.saveEvent({ ...baseData, date: dateVal }, id); UI.showToast("Actualizado"); }
    else {
      const repeatVal = parseInt(document.getElementById("eventRepeat").value) || 1; const baseDateObj = new Date(dateVal + "T12:00:00"); const batch = [];
      for (let i = 0; i < repeatVal; i++) { let nextDate = new Date(baseDateObj); nextDate.setMonth(baseDateObj.getMonth() + i); batch.push(FirebaseService.saveEvent({ ...baseData, date: Cal.formatDateISO(nextDate) })); }
      await Promise.all(batch); UI.showToast("Guardado");
    }
    UI.closeModals(); const [y, mm, d] = dateVal.split('-'); State.selectDate(parseInt(y), parseInt(mm)-1, parseInt(d)); State.currentDate = new Date(State.selectedDate); State.notify();
  } catch(e) { UI.showToast("Error", "error"); } finally { btn.textContent = "Guardar"; btn.disabled = false; }
}

async function saveAbsence() {
  const date = document.getElementById("absDate").value; const resp = document.getElementById("absResponsible").value;
  if(!date || !resp) return UI.showToast("Seleccione fecha y colaborador", "error");
  try { await FirebaseService.saveAbsence({ date, responsible: resp, type: document.getElementById("absType").value, obs: document.getElementById("absObs").value }); UI.showToast("Ausencia registrada", "warning"); UI.closeModals(); } catch(e) { UI.showToast("Error", "error"); }
}

function updateNotifButtonUI() { const btn = document.getElementById("btnNotifToggle"); if (Notification.permission === "granted") { if (State.notificationsEnabled) { btn.innerHTML = "✅ Avisos"; btn.className = "btn-icon bg-blue-light text-blue text-bold border-blue pl-md pr-md"; silentAudio.play().catch(()=>{}); } else { btn.innerHTML = "🔕 Pausado"; btn.className = "btn-icon bg-danger-light text-danger border-danger pl-md pr-md"; silentAudio.pause(); } } else if (Notification.permission === "denied") { btn.innerHTML = "🚫 Bloqueado"; } else { btn.innerHTML = "🔔 Activar"; } }
function toggleNotifications() { if (!("Notification" in window)) return alert("Tu navegador no soporta notificaciones."); if (Notification.permission !== "granted") { Notification.requestPermission().then(perm => { if (perm === "granted") { State.notificationsEnabled = true; localStorage.setItem('notificationsEnabled', 'true'); UI.showToast("¡Activado!"); updateNotifButtonUI(); } }); } else { State.toggleNotifications(); UI.showToast(State.notificationsEnabled ? "Activado" : "Pausado"); updateNotifButtonUI(); } }
function startNotificationPolling() { setInterval(() => { if (Notification.permission !== "granted" || !State.notificationsEnabled) return; const now = new Date(); const currentTimeString = `${String(now.getHours()).padStart(2, '0')}:${String(now.getMinutes()).padStart(2, '0')}`; const todayStr = Cal.formatDateISO(now); State.events.forEach(evt => { if (evt.date.startsWith(todayStr) && evt.time === currentTimeString && !evt.completed && !evt.notified) { UI.showNotificationCard(evt.title, `🔔 AHORA: ${evt.responsible}`); const notif = new Notification(`⏰ ${evt.time} - ${evt.title}`, { body: `Responsable: ${evt.responsible}`, icon: "https://cdn-icons-png.flaticon.com/512/2693/2693507.png", silent: true }); notif.onclick = function() { window.focus(); this.close(); }; evt.notified = true; setTimeout(() => { evt.notified = false; }, 65000); } }); }, 5000); }
